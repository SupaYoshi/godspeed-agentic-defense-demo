...
Godspeed currently has a live dev/test Foundry integration path. The dev/test Foundry IQ knowledge-grounding proof has been captured, and the Foundry OpenAPI tool path is working through the public Godspeed backend.
...
- Secrets: not stored in the repository or browser JavaScript.
...
7. Capture the minimum Foundry IQ screenshots before attempting tools.
...
12. Use `microsoft/foundry-iq-tenant-evidence-checklist.md` to confirm the complete screenshot set and keep production claims separate from dev/test proof.

## What Not To Claim Yet

Do not claim production grounding, production remediation, Defender/Sentinel/Intune integration, Work IQ, Fabric IQ or Microsoft Graph grounding from the Foundry IQ screenshots alone.

Use this wording after the captured knowledge-grounding proof and successful OpenAPI tool call:

> Godspeed includes dev/test Microsoft Foundry proof using `godspeed-defense-mission-knowledge` with the Godspeed Foundry Agent, plus the configured `godspeed_mission_api` OpenAPI tool for governed mission package creation.
